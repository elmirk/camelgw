...
/* 			//		event_flags = event_flags | (unsigned short) event_BIT2_ON; /\*calledBCD bit set ON*\/ */
...
/* 		if (status == IN_SUCCESS) */
/* 		    { */
/* 			printf("forwarding pending from IDP\n"); */

/* 			//need to move to prepaid so libCAMELGW_invoke_continue(ic_dlg_id, dlg_ptr, h, &invokeID);	  */
...
printf("debug:intu_handle_idp:m2m:out of order\n");
  ...
  /* 	} */
	...
	//	printf("strong service mode = PREPAID!\n");
	//	service_mode = MODE_PREPAID;
